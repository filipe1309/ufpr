class EditionsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @edition.save
        format.html { redirect_to @edition, notice: 'Edicao adicionada.' }
        format.json { render action: 'show', status: :created, location: @edition }
      else
  ...
  def update
    respond_to do |format|
      if @edition.update(edition_params)
        format.html { redirect_to @edition, notice: 'Edicao atualizada.' }
  ...
  private
    ...
    def edition_params
      params.require(:edition).permit(:title, :book_id, :number)
    end
end
